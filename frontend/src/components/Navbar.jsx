    ...
    <nav className="sticky top-0 z-50 bg-[#0d1321]/80 backdrop-blur-2xl border-b border-white/[0.05]">
      <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 bg-white/[0.03] p-1 rounded-2xl border border-white/[0.05]">
          ...
          <span className="text-gray-600 text-xs px-3 py-1 bg-white/[0.02] rounded-lg border border-white/[0.05]">
            {user?.username} <span className="text-gray-500">({user?.role})</span>
          </span>
          <button onClick={onLogout} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-all duration-300">
            <LogOut size={16} />
          </button>
        </div>
      </div>
